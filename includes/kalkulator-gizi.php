...
class WHONutritionCalculatorRevised {
    
    private $whoStandards;
    private $reasonableLimits;

    public function __construct() {
        // Data standar WHO tetap sama, ini adalah inti dari kalkulator Z-score
        $this->whoStandards = $this->initializeWHOStandards();
        
        // Aturan baru: definisikan batas kewajaran untuk validasi prioritas
        // CATATAN PENTING: Nilai ini sebaiknya divalidasi dengan data WHO yang lebih lengkap.
        // Ini adalah contoh untuk rentang usia 6-24 bulan secara umum.
        $this->reasonableLimits = [
            // format: 'gender' => ['param' => [min, max]]
            'boys'  => ['weight' => [5.0, 15.0], 'height' => [62.0, 92.0]],
            'girls' => ['weight' => [4.5, 14.5], 'height' => [60.0, 90.0]]
        ];
    }
    
    public function calculateNutritionalStatus($gender, $ageMonths, $weight, $height) {
        // =================================================================
        // LANGKAH 1: VALIDASI PRIORITAS UTAMA (ATURAN BARU)
        // =================================================================
        $limits = $this->reasonableLimits[$gender] ?? null;

        if ($limits) {
            $is_under_limit = ($weight < $limits['weight'][0] || $height < $limits['height'][0]);
            $is_over_limit  = ($weight > $limits['weight'][1] || $height > $limits['height'][1]);

            if ($is_under_limit || $is_over_limit) {
                $summary_text = '';
                if ($is_under_limit && $is_over_limit) {
                    $summary_text = 'Kondisi Langka: Pertumbuhan tidak seimbang (Segera Periksa ke Dokter)';
                } elseif ($is_under_limit) {
                    $summary_text = 'Kondisi Tidak Normal (Segera Periksa ke Dokter)';
                } elseif ($is_over_limit) {
                    $summary_text = 'Kondisi Langka (Obesitas/Faktor Genetik). Segera periksa ke dokter.';
                }
        ...
        // =================================================================
        // LANGKAH 2: JIKA LOLOS VALIDASI, LANJUTKAN PERHITUNGAN Z-SCORE
        // =================================================================
        try {
            $genderData = $this->whoStandards[$gender];
            
            // Hitung Z-score untuk setiap indikator
            $z_wfa = $this->getZScoreForIndicator('weight_for_age', $genderData, $ageMonths, $weight, $height);
            $z_hfa = $this->getZScoreForIndicator('height_for_age', $genderData, $ageMonths, $weight, $height);
            $z_wfh = $this->getZScoreForIndicator('weight_for_height', $genderData, $ageMonths, $weight, $height);

            // =================================================================
            // LANGKAH 3: KONVERSI Z-SCORE MENJADI KODE STANDAR (ATURAN BARU)
            // =================================================================
            return [
                'success' => true,
                'is_special_case' => false,
                'results_code' => [
                    'wfa' => $this->getCategoryCode($z_wfa, 'wfa'), // BB/U
                    'hfa' => $this->getCategoryCode($z_hfa, 'hfa'), // TB/U
                    'wfh' => $this->getCategoryCode($z_wfh, 'wfh')  // BB/TB
                ]
            ];

        } catch (Exception $e) {
            return ['success' => false, 'error' => 'Gagal menghitung Z-score: ' . $e->getMessage()];
        }
    }

    private function getZScoreForIndicator($indicator, $genderData, $ageMonths, $weight, $height) {
        $key = ($indicator === 'weight_for_height') ? round($height * 2) / 2 : $ageMonths; // Round to nearest 0.5 for height
        $value = ($indicator === 'height_for_age') ? $height : $weight;
        
        $lmsData = $this->interpolateLMS($genderData[$indicator], $key);
        if (!$lmsData) {
            throw new Exception("Data LMS tidak ditemukan untuk indikator {$indicator} dengan kunci {$key}.");
        }

        return $this->calculateZScore($value, $lmsData[0], $lmsData[1], $lmsData[2]);
    }

    private function getCategoryCode($zscore, $type) {
        $z_kurang = -2; // Di bawah -2 SD
        $z_lebih = 2;   // Di atas +2 SD
        
        switch($type) {
            case 'wfa': // Berat Badan per Umur
                if ($zscore < $z_kurang) return 'K1'; // Kurang
                if ($zscore > $z_lebih) return 'K3'; // Lebih
                return 'K2'; // Normal
            
            case 'hfa': // Tinggi Badan per Umur
                if ($zscore < $z_kurang) return 'T1'; // Pendek (stunted)
                if ($zscore > $z_lebih) return 'T3'; // Tinggi
                return 'T2'; // Normal

            case 'wfh': // Berat Badan per Tinggi Badan
                if ($zscore < $z_kurang) return 'WH1'; // Kurus (wasted)
                if ($zscore > $z_lebih) return 'WH3'; // Gemuk/Obesitas
                return 'WH2'; // Normal
        }
    }

    // =================================================================
    // FUNGSI INTI UNTUK PERHITUNGAN (TIDAK PERLU DIUBAH)
    // =================================================================
    ...
    private function interpolateLMS($data, $key) {
        ...
        return [
            $data[$lowerKey][0] + $ratio * ($data[$upperKey][0] - $data[$lowerKey][0]), // L
            $data[$lowerKey][1] + $ratio * ($data[$upperKey][1] - $data[$lowerKey][1]), // M
            $data[$lowerKey][2] + $ratio * ($data[$upperKey][2] - $data[$lowerKey][2])  // S
        ];
    }
    
    private function initializeWHOStandards() {
        return [
            'boys' => [
                'weight_for_age' => [
